...
# Judge 프롬프트 템플릿
JUDGE_PROMPT = """You are an expert evaluator of creative writing. Your task is to evaluate and rank three essay responses on THREE SEPARATE criteria: creativity, lexical diversity, and coherence.
...
**Instructions:**
- For EACH criterion (creativity, lexical_diversity, coherence), rank the three essays separately from 1st to 3rd place
        ...
        for line in f:
            ...
            if line:
                try:
                    ...
                    if data.get('continuation') and data.get('continuation').strip():
                        results.append(data)
...
def judge_single_essay_trio_threaded(comparison_id, essay_idx, essay_dof, essay_zero_shot, essay_few_shot, model_name, max_retries=3):
    try:
        judge_llm = initialize_judge_llm(model_name)
        judge_chain, _ = create_judge_chain(judge_llm)
...
def validate_judge_result(result: Dict[str, Any]) -> bool:
    try:
        ...
        for criterion in criteria:
            ...
            if not all(pos in ranking for pos in ['1st', '2nd', '3rd']):
                return False
            
            essays = set(['A', 'B', 'C'])
            ranked_essays = set([ranking['1st'], ranking['2nd'], ranking['3rd']])
            ...
            if not all(essay in scores for essay in ['A', 'B', 'C']):
                return False
...
def convert_rankings_to_scores(result: Dict[str, Any]) -> Dict[str, Dict[str, int]]:
    method_map = {'A': 'dof', 'B': 'zero_shot', 'C': 'few_shot'}
    criteria = ['creativity', 'lexical_diversity', 'coherence']
    
    scores = {}
    
    for criterion in criteria:
        scores[criterion] = {'dof': 0, 'zero_shot': 0, 'few_shot': 0}
        ranking = result[criterion]['ranking']
        
        scores[criterion][method_map[ranking['1st']]] = 3
        scores[criterion][method_map[ranking['2nd']]] = 2
        scores[criterion][method_map[ranking['3rd']]] = 1
...
def run_judge_evaluation(
    ...
    print(f"\n{'='*60}")
    print(f"LLM-as-a-Judge: Separate Evaluation of Creativity, Lexical Diversity & Coherence")
    ...
    if len(result_files) != 3:
        missing = set(['dof', 'zero_shot', 'few_shot']) - set(result_files.keys())
    ...
    min_essays = min(len(results[method]) for method in results)
    if min_essays < n_comparisons:
        print(f"Warning: Only {min_essays} essays available, reducing comparisons to {min_essays}")
    ...
    for i, idx in enumerate(indices):
        ...
        tasks.append((i, idx, essay_dof, essay_zero_shot, essay_few_shot, model_name, 3))
    ...
    with ThreadPoolExecutor(max_workers=num_processes * threads_per_process) as executor:
        future_to_task = {
            executor.submit(judge_single_essay_trio_threaded, *task): task 
            for task in tasks
        }
    ...
    for criterion in criteria:
        criteria_stats[criterion] = {
            'rankings_count': {'dof': {'1st': 0, '2nd': 0, '3rd': 0},
                              'zero_shot': {'1st': 0, '2nd': 0, '3rd': 0},
                              'few_shot': {'1st': 0, '2nd': 0, '3rd': 0}},
            'total_scores': {'dof': 0, 'zero_shot': 0, 'few_shot': 0},
            'detailed_scores': {'dof': [], 'zero_shot': [], 'few_shot': []}
        }
    ...
    for criterion in criteria:
        ...
        criteria_stats[criterion]['average_detailed_scores'] = {
            method: np.mean(scores) if scores else 0
    ...
    results_summary = {
        ...
        'performance': {
            ...
            'comparisons_per_second': n_successful / (end_time - start_time),
            'success_rate': n_successful / len(evaluation_results) * 100
        }
    }
    ...
    results_file = output_dir / f"judge_results_separate_criteria_{timestamp}.json"
...
def create_visualization(results_summary: Dict[str, Any], output_dir: Path):
    ...
    methods = ['DoF', 'Zero-shot', 'Few-shot']
    method_keys = ['dof', 'zero_shot', 'few_shot']
    ...
    for i, (criterion, criterion_label) in enumerate(zip(criteria, criteria_labels)):
        ...
        ax1 = axes[i, 0]
        ax1.bar(x, first_place, width, label='1st Place (3 points)', color='gold', alpha=0.8)
        ax1.bar(x, second_place, width, bottom=first_place, label='2nd Place (2 points)', color='silver', alpha=0.8)
        ax1.bar(x, third_place, width, bottom=np.array(first_place) + np.array(second_place), 
                label='3rd Place (1 point)', color='#CD7F32', alpha=0.8)
        ...
        ax2 = axes[i, 1]
        bars = ax2.bar(methods, avg_ranking_scores, color=['skyblue', 'lightgreen', 'lightcoral'], alpha=0.8)
        ax2.set_xlabel('Methods')
        ax2.set_ylabel('Average Ranking Score')
        ax2.set_title(f'{criterion_label}: Avg Ranking Scores')
        ax2.set_ylim(0, 3)
        ...
        ax3 = axes[i, 2]
        bars = ax3.bar(methods, avg_detail_scores, color=['skyblue', 'lightgreen', 'lightcoral'], alpha=0.8)
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    fig_path = output_dir / f"separate_criteria_evaluation_{timestamp}.png"
...
def print_summary_stats(results_summary: Dict[str, Any]):
    print(f"\n{'='*80}")
    print(f"SEPARATE EVALUATION SUMMARY: CREATIVITY, LEXICAL DIVERSITY & COHERENCE")
    print(f"{'='*80}")
    ...
    for criterion, criterion_label in zip(criteria, criteria_labels):
        ...
        print("-" * 65)
        print(f"{'Method':<12} {'1st':<8} {'2nd':<8} {'3rd':<8} {'Avg Score':<12} {'Avg Detail':<10}")
        print("-" * 65)
        ...
        for method, key in zip(methods, method_keys):
            ...
            print(f"{method:<12} {first:<8} {second:<8} {third:<8} {avg_score:<12.2f} {avg_detail:<10.1f}")
        ...
        if total_comparisons > 0:
            best_method_idx = avg_scores.items()
            best_method = max(best_method_idx, key=lambda x: x[1])
    ...
    overall_wins = {'dof': 0, 'zero_shot': 0, 'few_shot': 0}
    for criterion in criteria:
        criterion_data = criteria_stats[criterion]
        rankings_count = criterion_data['rankings_count']
    ...
    for method, key in zip(methods, method_keys):
        ...
        win_percentage = (total_wins / (total_comparisons * 3)) * 100 
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LLM-as-a-Judge: Separate Evaluation of Creativity, Lexical Diversity & Coherence")
